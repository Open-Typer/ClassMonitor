...
/*! Returns list of exercises the student has used. */
QList<int> classManager::studentExercises(int classID, int id, QString pack, int lesson, int sublesson)
{
	QList<int> out;
	out.clear();
	QDirIterator it(fileUtils::configLocation() + "/classes/" + QString::number(classID) + "/student_" + QString::number(id) + "/stats/" + pack);
	...
	while(it.hasNext())
	{
		...
		if(fileInfo.isDir() && (fileInfo.fileName() != ".") && (fileInfo.fileName() != ".."))
		{
			...
			if(lID == lesson)
			{
				...
				if(sID == sublesson)
				{
					int eID = exerciseID(fileInfo.fileName(),3);
					if(!out.contains(eID))
						out += eID;
				}
			}
...
/*! Converts integer list of exercises to a human-readable string list. */
QStringList classManager::exerciseList(QList<int> in)
{
	QStringList out;
	out.clear();
	for(int i=0; i < in.count(); i++)
		out += tr("Exercise") + " " + QString::number(in[i]);
	return out;
}
...
QStringList classManager::historyEntry(int classID, int studentID, QString pack, int lesson, int sublesson, int exercise, int entry)
{
	QFile historyFile(fileUtils::configLocation() + "/classes/" + QString::number(classID) + "/student_" + QString::number(studentID) + "/stats/" + pack + "/" +
		QString::number(lesson) + "." + QString::number(sublesson) + "." + QString::number(exercise) + "/history.csv");
	if(historyFile.open(QIODevice::ReadOnly | QIODevice::Text))
	{
		...
		while(!historyFile.atEnd())
		{
			QString currentLine = historyFile.readLine();
			if(line == entry)
			{
				QStringList out;
				out.clear();
				QString part = "";
				for(i=0; i < currentLine.count(); i++)
				{
					if(currentLine[i] == ';')
					{
						out += part;
